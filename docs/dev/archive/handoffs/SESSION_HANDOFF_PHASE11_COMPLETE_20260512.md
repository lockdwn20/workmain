...
| 3 | ClientRepository with atomic set_active(); migrate_client_attribution.py script; ACME seeded as active client | 98198cb |
...
| `scripts/migrate_client_attribution.py` | v1.0 | 3 | One-time script: backfills existing rows to ACME client_id |
...
3. `_restore_acme_active()` after each test to restore production state
...
feat(phase11): Gate 3 — client_repository, data attribution script, ACME seeded
