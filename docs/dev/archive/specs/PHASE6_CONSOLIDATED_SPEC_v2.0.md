...
1. Move existing report files:

```bash
mv reports/*.md output/reports/ 2>/dev/null || true
```

1. Update `.gitignore` — replace existing `reports/` entry with:
...
1. Update all path references in `report.py` from `reports/` to `output/reports/`

2. Verify `workmain report list` still shows all existing reports
3. Verify `workmain report show <filename>` still works
4. Verify `workmain report costs` still shows correct totals
5. Remove old `reports/` directory if empty
...
```
To: peter@example.com, benjamin@example.com
CC: rabbit@example.com
...
2   benjamin@example.com        to       --       --
3   rabbit@example.com         --       to       to
4   bunny@example.com       cc       cc       --
