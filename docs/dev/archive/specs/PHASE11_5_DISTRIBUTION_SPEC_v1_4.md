...
- v1.5  Replaced certain data values as part of an application wide update
...
- Active client = ACME → recipient gets `client_id = ACME.id`
...
recorded at Phase 11 Gate 0 (`"#int-acme"`). Confirm active client
...
COMMENT ON COLUMN clients.slack_channel IS
    'Slack channel for this client (e.g. #int-acme). '
...
- Positional: `channel` — Slack channel name (e.g. `#int-acme` or
  `int-acme` — normalize by adding `#` if absent)
...
1. Confirm `config.json` now contains only `workspace_name`
...
- Display format: `"Channel: #int-acme (Client: ACME)"`
...
[ ] SELECT slack_channel FROM clients WHERE is_active = TRUE — shows
    "#int-acme" (migrated value confirmed)
