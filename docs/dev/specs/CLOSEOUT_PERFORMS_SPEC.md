...
- **DR10 — The skill is a directory, and the branch types are its variants.** `SKILL.md` carries only what every run needs: when to run it, the preflight table, the two stops, and how to pick the variant. Each branch type's perform sequence is a reference file read once that type resolves, so a `chore/*` run never loads the `feature/*` PR-wait procedure. This is `skill-creator`'s progressive-disclosure and per-variant-reference pattern, and the three workpaths are exactly the shape it describes.

  **Every step states the rule that requires it.** The retired skill was five steps of "run the script, compare its output to the table" — no reasoning, no procedure, nothing an operator could act on when a check failed. That is why it could not survive its first real branch: with nothing but a comparison to fall back on, a failed run had no next move (Ray, 20260819). Instructions here are imperative and cite the standard behind them, so a reader meeting an unforeseen case can reason from the rule rather than pattern-match against a command list. A bare `MUST` with no reason behind it is a defect in this skill, not a strength.
...
| AC3.8 | No reference file is a bare command list — every step cites the rule behind it | In each of the three files, `grep -c '§'` is at least the number of numbered steps, counted by `grep -cE '^[0-9]+\. '`. This is a floor, not a proof: a padded file could pass it, but a list of commands with no reasoning cannot. It exists because "run the script, compare the table" is the defect this spec replaces | 5th |
