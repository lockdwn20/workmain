...
| 20260831 | Caliper | §2's evidence for `clockify sync push` was a zero-hit grep on `clockify.py` — the wrong file. The prompts live in `ClockifySync` (`sync.py:284,328`); the conclusion held only because both sit behind `pull_entries`, which `push_entries` never calls. | Accepted. §2 now cites the push/pull split as the evidence. A grep scoped to the command's own module is not a transitive sweep, which is the same boundary error as the `meetings condense` finding above. |
...
| The remaining invoked children contain no prompt, transitively — command body and every helper it calls were both checked. | `reports.py:257-283` → `generate_report_impl` (`reports.py:112-236`); `email.py:292-330` → `_generate_draft` (`email.py:47-172`); `time.py:480-640` (the prompts at `time.py:134,189,262,333,349,466` are all in `time add`/`edit`/`delete`) |
| `clockify sync push` is prompt-free, but not because `clockify.py` is: `ClockifySync` prompts in `_resolve_conflict_interactive` (a bare `input()` at `sync.py:284`) and `_import_clockify_entry` (`click.prompt` at `sync.py:328`). Both are reached only from `pull_entries`, at `sync.py:174` and `:192`. `push_entries` spans `sync.py:59-131` and calls neither. | as cited |
| `clockify report save` reaches only `client.py` and `auth.py`, which contain no prompt. | as cited |
...
- **DR3 — Every call passes an explicit `timeout=`, and the bound is per call, not per step.** `run_workmain()` has no default timeout and the parameter is keyword-only and required. A caller that does not know its bound is given one in §4's table; it is never allowed to omit it.
  - A step issuing more than one call is bounded at N × its timeout, not at its timeout. Three cases exist: `_run_condense_step` loops site 213 once per pending meeting, and the retry pairs at 892/903, 1043/1057 and 1083/1096 each run two. A five-meeting condense step can therefore hold the daemon thread for 2.5 hours at `TIMEOUT_AI`, and a report step for one.
  - This is accepted here. It is bounded where today it is unbounded, which is what issue #94 asks for. A step-level deadline is a different mechanism — a monotonic clock threaded through the loop, plus a decision about what a half-condensed step returns — and belongs in its own issue rather than growing this one.
...
| A step that issues several calls is bounded at N × its timeout, so the worst-case daemon-thread hold is much longer than any single constant suggests. | The condense step scales with the number of pending meetings; three report/email/clockify steps are bounded at 2×. | Stated in DR3 rather than fixed. Each call is individually bounded, so the thread always makes progress or fails — it never hangs. A step-level deadline is a follow-up issue. |
