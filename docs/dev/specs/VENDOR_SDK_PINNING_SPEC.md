...
| 20260903 | Caliper | **F1 (second part)** — specifying that Gemini retries a 400 three times bakes in the asymmetry with `ClaudeProvider`, which fails fast on a 4xx other than 408/409/429 | Acknowledged in §1 Out of scope and raised with Ray as **Q8**, unanswered. This spec preserves today's Gemini behaviour; alignment is a behaviour change the issue does not ask for and Spanner will not make silently. |
...
- **The 4xx retry asymmetry between the two providers.** `ClaudeProvider.generate()` fails fast on a 4xx other than 408/409/429 (`claude.py:162-176`); `GeminiProvider` retries every non-429 failure, and this spec preserves that. Aligning them is a behaviour change #126 does not ask for — **Q8**, raised with Ray, unanswered. If he says align, DR2 gains one condition and AC5.1's wording changes; nothing else in this spec moves.
...
| The DR3 pin set resolves cleanly | same command on the candidate file, exit 0 | live PyPI |
| Nothing under `workmain/` imports `pydantic` or `httpx`; the only first-party `httpx` use is a test helper | `grep -rn 'pydantic\|httpx' workmain/ tests/` → `tests/test_ai_clients.py:367,407-408` | source |
| `anthropic.APIStatusError` still constructs from an `httpx.Response`, so `_status_error` survives | probe with `anthropic==1.3.0` + **`httpx==0.28.1`**, the shipped pair → `status_code == 429` | **shipped set** |
...
- **DR7 — Nothing but the classification changes.** A 429 raises `RateLimitError` immediately, as today. `TypeError` → `GenerationError`, as today. Every other failure retries with the same backoff and ends as the same `GenerationError`, as today. This spec changes what is *classified* as a rate limit and where the clause sits — not what happens to anything else.
...
| 5 | Capture `pip freeze > /tmp/workmain-pre-upgrade.txt` **before** anything is installed. Then, at the authorization point below, upgrade the working `.venv` from the new pins and run the full suite against it. Build a second, throwaway virtualenv from the same `requirements.txt` and `diff` its `pip freeze` against the upgraded `.venv`'s — this is what produces AC2. Any test needing adjustment under the new SDKs is fixed and committed here; if none does, the step commits nothing and says so. | `.venv` and a throwaway venv (both untracked), possibly `tests/**` |
...
| AC2.2 | Two independently built environments install the same versions — the property "deterministic" names | `diff` of the throwaway venv's `pip freeze` against the upgraded `.venv`'s, from Step 5, is empty |
...
| AC4.2 | The tests assert on the one attribute the provider reads and never call the vendor constructor, so they hold across the upgrade rather than against one SDK's signature (DR6) | `grep -n 'ClientError(' tests/test_ai_clients.py` returns zero hits, and the Step 2 tests are green both before Step 3 and again in Step 5 |
| AC5.1 | No failure other than a 429 becomes a `RateLimitError` — a 400 takes the retry path and ends as `GenerationError` | `pytest tests/test_ai_clients.py::TestGeminiRateLimitTranslation::test_generate_400_retries_and_raises_generation_error` |
...
| `test_generate_400_retries_and_raises_generation_error` | non-429 vendor error takes the retry path, ends as `GenerationError` |
