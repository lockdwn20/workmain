...
| C15 | Live old-path citations are `docs/dev/specs/ISSUE_CREATION_VALIDATION_SPEC.md:87` and `:102`, plus this spec and its own recon, which quote the old path as the subject of the work and keep it — that is why AC5.1 excludes them by name rather than by directory. The archive hit is `docs/archive/design/DESIGN_PLANNING_DOCS_STANDARDS_REVIEW.md:329` — repository-wide grep |
...
**Verification run.** AC4.1 owns this: capture the script's output before the move as the first action of step 1, capture it again from `automation/` at step 2, and record both in the step 2 commit message. There is nothing in §2 to compare against — C17 deliberately carries no output.
...
- The two behavioural checks that are not pytest — AC2.2's offline hook invocation and AC4.1's before/after comparison — are run once and recorded in their step's commit message, which is where a `chore/*` branch's evidence lives. Neither touches the network.

## 7. Risks and rollback

| Risk | Mitigation |
| --- | --- |
| The hook stops protecting `main` because a path was missed. This is the whole point of the issue, and today it happens silently (C8) | AC2.1 greps both directions, and DR3 removes the silence: after step 2 a missing checker is a rejected push with a message. AC2.2 proves it by driving the hook directly over crafted stdin, with no push and no network |
