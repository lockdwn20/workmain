...
| C17 | Current output of `python3 scripts/check_release_integrity.py --no-remote`, captured at authoring time, exit `0`: `OK — 5 release(s) at or above the v1.26.0 baseline are consistent [tags vs CHANGELOG]; __version__.py = 1.29.0` followed by `13 pre-baseline item(s) accepted as history (not failed). Run with --show-historical to list them.` |

## 3. Design rules

- **DR1 — the move is a `git mv`, not a delete-and-create.** History follows the file. Nothing outside the four files named in §1 is touched.
...
| AC1.2 | History followed the file | `git log --follow --oneline automation/check_release_integrity.py \| wc -l` prints more than `1` |
...
| AC2.2 | A missing checker fails loudly instead of exiting zero | `grep -c '|| exit 0' .githooks/pre-push` prints `0`. Behavioural check: with the checker temporarily renamed, `git push` to `main` or a tag is rejected with a message naming the missing file — run once and record in the step 2 commit message, restoring the file immediately |
...
| AC4.1 | The script runs from its new location with the same output as before the move | `python3 automation/check_release_integrity.py --no-remote` prints the two lines quoted in C17 and exits `0`. Compare against C17 verbatim |
...
| AC5.1 | No live old-path citation remains outside the archive and the parked branch | `grep -rn 'scripts/check_release_integrity' . --exclude-dir=.git --exclude-dir=archive` returns no hits |
