...
| 20260902 | Spanner | Design study Q4 — `count_tokens` calls a method the SDK does not have; retry multiplication; dead config keys. | Out of scope. `count_tokens` becomes its own issue at close-out. |
...
- `config/ai_settings.json` — the `providers.claude` block: `model`, both `cost_per_1k_*`, `cost_structure`, `notes`, `last_updated`.
...
- `ClaudeProvider.count_tokens()` (broken against `anthropic` 0.75.0), the provider-plus-SDK retry multiplication, and the dead `default_max_tokens` / `default_temperature` config keys. All recorded in `DESIGN_CLAUDE_PROVIDER_CURRENT_MODEL.md` F9–F11.
...
- **DR4 — a request the API will never accept is not retried.** A 4xx other than 408, 409 and 429 is a permanent property of the request: fail on the first attempt, surfacing the API's own message. 408/409/429 and every 5xx keep the existing backoff. `RateLimitError` keeps its dedicated handler ahead of the chain.
...
- **DR10 — a missing policy file is a configuration error, not a default.** A provider whose policy file is absent or unparseable raises `ConfigurationError` at construction. Silently falling back to a built-in default is how a payload contract goes stale unobserved.
...
| --- | --- | --- |
| 1 | `config/providers/claude_settings.json` and `gemini_settings.json`. Claude: `thinking: "disabled"`, `sampling: {}` (empty — nothing sent). Gemini: `sampling: {"temperature": "from_request"}`. Each file carries a `description` naming DR8's boundary. (DR7, DR8, DR9) | `config/providers/*.json` |
| 2 | Policy loading. `ProviderManager` reads `config/providers/<name>_settings.json` alongside the `ai_settings.json` section and passes it to the provider at construction; absent or unparseable raises `ConfigurationError`. (DR10) | `workmain/ai/provider_manager.py` |
...
| 5 | Fail fast on permanent failures — an `except APIStatusError` clause between the existing rate-limit and `APIError` clauses, per DR4. | `workmain/ai/providers/claude.py` |
...
| 7 | Offline tests, client patched, no network: no sampling key and `thinking == {"type":"disabled"}` in the `generate()` payload; the same for `check_availability()`; a 400 raises after exactly one call; a 500 still retries `retry_attempts` times; missing `model` raises `ConfigurationError`; a missing policy file raises `ConfigurationError`; Gemini's sampling comes from its policy file. | `tests/test_ai_clients.py` |
| 8 | Point config at a current model: `model: "claude-sonnet-5"`, costs `0.002` / `0.010`, `cost_structure` and `notes` rewritten, `last_updated` bumped. | `config/ai_settings.json` |
...
| AC2.1 | The two configuration files cannot drift into duplication: no key appears both in a provider's `ai_settings.json` block and in its policy file. (DR9) | key-set intersection over the two files returning empty |
...
| AC4.1 | A provider whose policy file is missing or unparseable fails loudly at construction rather than falling back to a built-in default. (DR10) | `pytest tests/test_ai_clients.py -k missing_policy` |
...
| AC12.1 | The deployed configuration names a current-generation model at its real price, so cost tracking is not silently wrong. | `python -c "import json;c=json.load(open('config/ai_settings.json'))['providers']['claude'];print(c['model'],c['cost_per_1k_prompt_tokens'],c['cost_per_1k_completion_tokens'])"` |
...
- **Expected after:** 982–986 passed. Roughly ten new tests in Step 7; no existing test deleted.
