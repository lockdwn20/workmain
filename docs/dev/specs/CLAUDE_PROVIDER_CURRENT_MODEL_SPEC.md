...
| 20260902 | Caliper | **S8** — "a key the provider requires" (DR10) is never defined and is not inferable from Step 1, so an implementer would have to invent the set. | **Accepted.** DR10 now derives it: required = every key that provider reads. Step 1 names them per file so the set has one home. |
| 20260902 | Caliper | **S9** — `ConfigLoader.load()` raises `FileNotFoundError` / `json.JSONDecodeError`, never `ConfigurationError`, so DR10, Step 2 and AC4.1 all assume a type the boundary does not produce. Verified by execution. | **Accepted.** Step 2 translates both at the boundary into `ConfigurationError` naming the file. |
| 20260902 | Caliper | **S10** — nothing sequenced Ray's price edit before the live check and the PR, and Step 8 wrote a copy of a live price into a document that forbids applying it. | **Accepted, both halves.** Ray's edit is now its own Step 9, upstream of the live check and the PR. The values are gone from the spec — Anvil reports the current published rate at the stop instead. Writing live state into a document is what the standing rule forbids. |
| 20260902 | Caliper | **S11** — `ConfigLoader.load()`'s path join and error types, and `_load_config`'s `except Exception` absorbing failures into `_disabled`, are load-bearing for the revised design but not cited in §2. | **Accepted.** Two rows added. |
| 20260902 | Caliper | AC13.1 could read red for a reason unrelated to the payload contract — a short preamble tripping `max_tokens` at 20. | Noted in §7. Not an AC change: `end_turn` is the right property; the note tells the implementer to re-run before concluding. |
...
| `_load_config` wraps provider construction in `try / except Exception` and absorbs any failure into `self._disabled`, so an exception raised inside a provider's `__init__` never reaches the caller — it becomes "Provider is disabled" at first use. DR10 and Step 2 depend on this. | `workmain/ai/provider_manager.py:311-318` |
| `ConfigLoader.load(name)` resolves `config_dir / f"{name}.json"`, so a nested `providers/claude_settings` joins correctly without changing its pattern. It raises `FileNotFoundError` when the file is absent and `required=True` (the default), and re-raises `json.JSONDecodeError` when the file will not parse — **never `ConfigurationError`**. Both verified by execution. | `workmain/config_manager/loader.py:34`, `:49-55`, `:60-72` |
...
| --- | --- | --- |
| 1 | `config/providers/claude_settings.json`, `gemini_settings.json` and `ollama_settings.json`. Claude: `{"thinking": {"type": "disabled"}, "sampling": {}}` — **both keys required** (DR10). Gemini: `{"sampling": {"temperature": "from_request"}}` — **`sampling` required**. Ollama: empty policy, **no required keys**, with a `description` recording that its generation parameters are Modelfile-baked and set outside this repository. Each file carries a `description` naming DR8's boundary. (DR7, DR8, DR11) | `config/providers/*.json` |
| 2 | Policy loading via `ConfigLoader.load('providers/<name>_settings')`, **before** the provider is constructed and therefore outside the existing `try/except Exception` that absorbs construction failures into `_disabled` — so an unusable policy surfaces instead of silently disabling a provider. `ConfigLoader` raises `FileNotFoundError` for an absent file and `json.JSONDecodeError` for one that will not parse — **neither is `ConfigurationError`** — so `ProviderManager` catches both at this boundary, and detects a missing required key, and re-raises `ConfigurationError` naming the file in each case. The loaded policy is passed to the provider at construction. (DR10, DR12) | `workmain/ai/provider_manager.py` |
...
| 8 | Point config at a current model: `model: "claude-sonnet-5"`, `notes` rewritten, `last_updated` bumped. Delete the unread `default_max_tokens` and `default_temperature` keys from the `claude` and `gemini` blocks. (DR9) **Do not touch `cost_per_1k_prompt_tokens`, `cost_per_1k_completion_tokens` or `cost_structure`** — those are Ray's, set whenever a model is updated. Stop, tell him the model changed, and report the vendor's currently published rate for it, read from the pricing page at that moment. This spec deliberately records no price: a live figure copied into a document goes stale and then disagrees with config while looking authoritative. | `config/ai_settings.json` |
| 9 | **Ray sets the three price fields.** Nothing downstream runs until they match the model — Step 10's cost line and the PR in Step 11 are both computed from them. | `config/ai_settings.json` (Ray) |
| 10 | Live verification: `workmain providers test claude`. Record content and `stop_reason` in the results artifact. | none |
| 11 | Version bump to v1.32.0, CHANGELOG entry, PR `dev` → `main`. | `workmain/__version__.py`, `CHANGELOG.md` |

### Authorization points

**None** as defined by `docs/DEVELOPMENT_STANDARDS.md` §1.4 — no migration, no GitHub object deletion, no force-push, no service run-state change. Step 11 opens the PR and stops; Ray merges. A post-merge restart applies — `feature/*` branch.
...
- **AC13.1 can read red for a reason that is not the defect.** `providers test` sends `max_tokens=20`; with thinking off the response should end on `end_turn`, but a model that opens with a short preamble trips `max_tokens` for reasons unrelated to the payload contract. The two look identical from outside. Re-run before concluding anything from a red Step 10.
- **Rollback overall:** ten Anvil commits plus Ray's price edit on a `feature/*` branch, no migration and no schema change. Step 8 should not outlive Step 3 — a current model in config without the payload fix is exactly the 400 in issue #79.
