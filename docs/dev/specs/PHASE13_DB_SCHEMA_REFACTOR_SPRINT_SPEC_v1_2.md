WorkmAIn
PHASE13_DB_SCHEMA_REFACTOR_SPRINT_SPEC v1.3
20260730
...
- v1.3  Replaced certain data values as part of an application wide update
...
1. **Issue D** — `preview_report()` missing client filter (identified in
   hotfix spec; low-effort; same file; no schema touch).

2. **`notes delete` guard** — user-friendly pre-check before `ON DELETE
...
- `"ACME Internal: Scheduled Wednesday scope meeting..."` — 2026-03-09 — 4 matches
