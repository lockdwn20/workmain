...
**Withdrawn risk.** An earlier draft named the `__init__.py` work's circular-import exposure as the top risk. §2 records that no package-level import exists for any of the 14 packages, so DR4 makes them all markers and Step 5 adds no import. AC9.1 checks that directly.
