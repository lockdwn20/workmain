...
- **DR11 — The census is durable tooling, not a one-shot.** It lives in `automation/` for two reasons. §6.3 requires it: `scripts/` is "utilities and demos, never tests", and AC1.2 requires this instrument to carry its own tests. And it outlives the branch — §1.5's new bullet carries no enforcement, so without a re-runnable check the class reopens with the next module written and nothing catches it. `automation/check_release_integrity.py` is the established precedent: a standards checker in `automation/`, with its own test file, invoked by `.githooks/pre-push`. Whether this one is likewise wired into the hook is **open — see §8**.
...
| --- | --- | --- |
| 1 | **Write** `automation/rule_census.py`, implementing DR8 (`--headers`) and DR9 (`--text`), and `automation/rule_census_test.py` alongside it — one fixture per class in both modes, plus a headerless file and a clean file, so a classifier that never matches fails its own tests. Fixtures are inline strings written to `tmp_path` (DR10). **Then run both modes** over the untouched tree and record the output as the branch-point census. Nothing in this step edits a censused file. | `automation/rule_census.py`, `automation/rule_census_test.py` |
...
## 8. Open question for Ray

**Should `rule_census.py` be wired into `.githooks/pre-push`?**

DR11 makes it durable tooling. Durable tooling that nobody runs is a document, and this issue exists because a rule that lived where nothing enforced it was read the wrong way for two sessions running.

- **Wire it, matching `check_release_integrity.py`.** The hook gains a second checker on the same trigger — a push of `main` — blocking a push whose tree fails the census. Cost: a developer can be blocked at push time by a header, and the hook is currently narrow by deliberate design, firing only on pushes that can damage the release record. A header is not the release record, so this widens what the hook is for.
- **Deliver it unwired.** It is re-run at close-out and whenever someone asks. Cost: nothing runs it on its own, so §1.5's bullet is enforced by memory — which is the condition that produced #134.
- **Wire it, but on its own trigger.** Fires on any push, not just `main`, and reports without blocking — visible on every push, never a wall.

**Recommendation: the third.** The rule is worth checking on every push, and blocking a push over a docstring is out of proportion to the harm, whereas a silent unwired script is out of proportion to how this issue started. It also keeps the release-record hook's narrow scope intact rather than widening it.

**This is not specified below and no step implements it.** Say which one and I will add it as a step; the spec is complete without it either way.
