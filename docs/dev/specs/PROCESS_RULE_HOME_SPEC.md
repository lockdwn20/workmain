...
| 194 `.py` files sit in `tests/` (65), `workmain/` (110), `automation/` (5) and `scripts/` (14); `config/` holds none, so the four-tree and five-tree `.py` counts are the same number. **179 carry a module docstring**. Of the 15 without one, 14 are the empty `__init__.py` files and the fifteenth is `scripts/sanitize_ics.py`. | `ast` census |
...
| The five trees hold **270 files**: 194 `.py`, 28 `.json`, 24 `.sql`, 16 `.md`, 6 `.ics`, 1 `.sh`, 1 `.txt`. | `find tests workmain automation scripts config -type f` |
...
| **Adjudication under volume.** Independently measured at roughly **770 candidate rows across ~119 files**, plus a no-hit row for every remaining file — call it 900 determinations, most of them legitimate text. A reviewer can start clearing rows without reading them. | The half of issue #134's second acceptance criterion this spec exists to satisfy. | DR8.2 removes the largest source of noise — limiting non-module docstrings to `TRIGGER` and `COMMAND` cuts roughly 400 rows to roughly 50, all of them content §3.5 and §5.6 require. AC1.2 requires a determination per row and a reason per "leave as is", not a verdict. AC12.1 names specific rows verified in §2 that must each carry one, including the originating instance, so a blanket clearance is visible. |
