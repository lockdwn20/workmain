...
| F5 | The record phase — spec `Shipped` plus results artifact — sits at step 1 in `chore` and `feature` but step 2 in `hotfix`, because `hotfix` bumps first so `Released as` can quote a real version while `feature` *derives* the version instead, calling the minor bump "deterministic under §2.5". Two different answers to one question. **Drift, not forced** | `chore.md:5`; `feature.md:5`; `hotfix.md:5,7` | Medium |
...
| Q | Question | Answer |
| --- | --- | --- |
| Q1 | Does #91 absorb the `DEVELOPMENT_STANDARDS.md` fixes for F1 and F2, or do they become their own issue? Spanner's view: F1 must ride along, because the skill cannot cite a delete rule that is wrong; F2 is separable | |
| Q2 | Reference files currently restate the standards so a session reads `SKILL.md` plus one file and needs nothing else. Citing §2.x instead adds a third read. Which is worth more — self-containment, or one home per rule? | |
| Q3 | Does the record phase settle on `feature`'s derive-the-version or `hotfix`'s bump-first (F5)? | |
| Q4 | Should `SKILL.md` state the stop **order** per variant, given F8's inversion, or keep naming the pair without ordering them? | |
