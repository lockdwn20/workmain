...
| Q | Question | Answer |
| --- | --- | --- |
| Q1 | Can a Claude Code skill halt mid-procedure, obtain Ray's approval, and resume in the same invocation? Everything about the shape of a performing close-out depends on this. If it can, the skill orchestrates and stops at F6's two authorization points. If it cannot, the close-out is a sequence of separately-invoked phases, and the spec must define the phase boundaries and how state carries across them. | |
| Q2 | `feature/*` cannot reach a terminal state in one run (F7): the `dev → main` PR is Ray's to merge, and bump, tag, Release and restart all sit downstream of it. Is a `feature/*` close-out defined as **two invocations** with a documented resume point, or does `/closeout` stop at "PR opened" and a separate skill finish the job after the merge? | |
| Q3 | Where do actions live (F5)? DR3's two-way split has no place for `git merge`, `git tag`, `gh release create` or `systemctl restart`. Does the script gain an action mode, does the skill shell them directly, or does a third module own them? | |
| Q4 | What is the relationship between an issue's ACs and its spec's numbered sub-ACs (F18)? #90's fourth AC requires it stated. The candidates are: sub-ACs are the spec's private decomposition and never appear at close-out; or the results artifact carries both, mapped; or issue ACs are retired in favour of spec sub-ACs. This governs both the results template and the script's comparison. | |
| Q5 | Does AC verification stay full-prose string matching (F17), or move to identifiers with the prose carried alongside? §4.4 argues the prose match catches rewording drift, and #90's fourth AC says "No check requires AC prose to be copied into the results artifact." These are in direct conflict and one must give. | |
| Q6 | Is `#90` one issue or several (Ray's call, per the issue body)? The candidate seams are: the scope inversion and action architecture (F1–F8); the pre-merge branch lifecycle and reporting defects (F9–F14); and the AC-set question with its template amendment (F15–F18). The third is separable and touches documents the other two do not. | |
| Q7 | What happens to `CYCLE_CLOSEOUT_SPEC.md` and `CYCLE_CLOSEOUT_RESULTS.md`? DR2 becomes wrong and the results artifact records a shipped state that #90 supersedes. §1.5 permits `Status: Superseded`, but §1.2 also says spec changes are surgical rather than wholesale rewrites, and this is a scope inversion rather than an edit. | |
| Q8 | The one available pre-merge fixture is the parked `chore/issue-84-queue-sequencing` (F26), and a performing close-out run against it would merge it and delete it — consuming the fixture and closing #84 in the process. Is that the intended demonstration of #90's fifth AC, or does the demonstration need a throwaway branch so #84's close-out stays a separate deliberate act? | |

## 6. Disposition

- Promoted to: *pending — spec not yet written, per §1.1 Analysis.*
