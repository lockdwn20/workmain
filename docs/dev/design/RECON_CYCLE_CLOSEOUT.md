...
| F9 | **No issue in any state carries a checked box.** `gh issue list --state all --limit 200 --json number,body -q '.[] \| select(.body\|test("- \\[x\\]";"i")) \| .number'` returns nothing. Checkbox state is therefore not maintained and carries no evidence about whether an AC is met | The command above | High |
...
| F16 | **GitHub carries no issue → commit link for this repo's `chore/*` work.** `closedByPullRequestsReferences` is `[]` for #81, #82 and #86 — all three were closed by hand, and `chore/*` merges locally to both `main` and `dev` without a PR per §2.2 | `gh issue view {81,82,86} --json closedByPullRequestsReferences` | High |
...
| F19 | It is **tag-wide, not issue-scoped.** It sweeps every tag on the repository; it has no notion of one issue, one branch, or one release. Nothing in it answers "did *this* work get its Release" | `scripts/check_release_integrity.py`, `main()` | High |
...
| F24 | §2.6 states the rule but names no file-path trigger. The results template does: *"Daemon restart (if `workmain/**` or `config/*` changed)"*. That predicate exists only in the template, not in the standard | `docs/dev/results/_TEMPLATE_RESULTS.md` §5; `docs/DEVELOPMENT_STANDARDS.md` §2.6 | Medium |
...
| # | Finding | Evidence | Severity |
| --- | --- | --- | --- |
| F26 | `_TEMPLATE_RESULTS.md` **already prescribes** what #83 asks a close-out to produce: §3 is an AC table with `Met / Not met / Carried` checked against delivered code, citing Item 32 as the reason it exists; §5 carries the test-suite result, live verification, and the daemon-restart confirmation. The header carries `**Status:**` | `docs/dev/results/_TEMPLATE_RESULTS.md` §3, §5 | High |
...
| F30 | **No results artifact exists for #81, #82 or #86** — the three most recently closed issues, all `chore/*`. `docs/dev/results/` holds two `SESSION_HANDOFF_*` documents, both for pre-migration feature sprints. Applied retroactively, #83's fifth AC fails all three | `ls docs/dev/results/` | High |
...
| Q | Question | Answer |
| --- | --- | --- |
| Q1 | Given F7/F9, does the close-out read all three AC shapes, or only F7(a)? Reading only the new shape restricts the skill to issues #81 and later — nearly all live work is F7(b). Reading all three means the parser carries a shape it will keep meeting for years | *pending* |
| Q2 | Given F12–F17, how does the close-out learn an issue's branch type? Three candidates were found and none is complete: the spec header's `**Branch:**` field reached via a non-uniform `**Originating item:**` back-link (F13/F14); the branch-name convention `<type>/issue-<N>-*`, which is unwritten and post-migration only (F17); or an argument the caller passes. F15's `#N` ambiguity affects the first | *pending* |
| Q3 | Given F18–F21, does the close-out invoke `check_release_integrity.py`, or does it carry its own issue-scoped release check? The script answers *"is the release record consistent repo-wide"*; #83's AC asks *"did this issue's work get its tag and Release"* (F19). If it invokes the script, #87's move (F35) becomes a prerequisite rather than a later tidy-up | *pending* |
| Q4 | Given F26/F29/F30, does the close-out **write** the `docs/dev/results/` artifact, or **verify** that one exists and refuse otherwise? #83's AC says "refuses to complete without" — verification. But F26 shows the template already prescribes precisely the AC table the skill produces, and F30 shows the artifact does not get written on its own. If it verifies only, the AC table is produced twice by two authors | *pending* |
| Q5 | Given F33/F34, what is the skill called? #83 ships before #85, so it sets the precedent for both. Close-out fits none of #80's three role verbs — it is the step *after* implementation, and it audits Anvil's work rather than performing it | *pending* |
| Q6 | Given F27/F28, what `Status:` does a results artifact carry while close-out is running? The template offers `Shipped \| Superseded`; §1.5 allows `Active`. Whether the fix is a template change, a standards change, or neither, is a scope call for this spec | *pending* |
| Q7 | Given F30, is the gap retroactive? #81, #82 and #86 closed with no results artifact. Whether they are backfilled, grandfathered, or left as the reason the skill exists, changes what "cycle close-out" means for work already closed | *pending* |
| Q8 | Given F16/F29, is a durable issue ↔ artifact link created as part of this work — an `**Originating item:**` field on results artifacts, an issue comment naming the artifact, or neither? Without one, every close-out re-derives the link by grep | *pending* |
