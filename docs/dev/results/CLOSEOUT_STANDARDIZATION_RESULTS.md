...
| `docs/dev/design/`, `docs/dev/specs/` | Two specs advanced `Approved` → `Shipped`; four recons `Active` → `Shipped`; one `Superseded`; every stale Disposition repointed at the artifact it produced. At close-out step 1, `DESIGN_CLOSEOUT_STANDARDIZATION.md` advanced to `Shipped` and its Disposition — the last one still reading *"pending"* — was repointed at #91 and at AC14's walk-through | AC11, AC12 |
...
| AC11 | Met | `grep -c "^\*\*Status:\*\*"` returns 1 for every artifact in `docs/dev/{design,specs,results}/`, and every value is one §1.5 defines — the three `_TEMPLATE_*.md` carrying their option lists as the field's value, which is what a template is. Each status matches whether its work has shipped; the set is derived by that grep rather than counted here, so the row does not go stale as artifacts are added. `DESIGN_CLOSEOUT_STANDARDIZATION.md` advanced `Active` → `Shipped` at close-out step 1, with the rest |
...
| 11 | **`.wtest` deleted — a zero-byte stray tracked into the repository root** | Found at close-out, running P11's diff against the merge base. Committed in `a9396fb`, referenced by nothing, not ignored, and outside every AC on this issue. Left in place it would have shipped to `main` and `dev` on the merge, since a `chore/*` branch merges to both | Ray, 20260827 |
