...
| AC3.1 | Carried to Ray's reading | Property of a document; check is Ray's reading of the three `config/providers/*.json` files for whether any key asserts a model capability. Each file's only keys are `description`, `thinking`, `sampling`; the `description` states the send-not-support boundary explicitly. |
