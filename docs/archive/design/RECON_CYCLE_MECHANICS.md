...
| F32 | GitHub **auto-populates** the built-in `Status` field to `Todo` on every item added. `Status` cannot be deleted, only hidden in a view — **and hiding it in the Web UI view does not remove it from `gh project item-list --format json` output**, which still emits `status` per item. Disposition: ignored. Nothing reads it, nothing writes it | `gh project item-list --format json`; Ray hid the column and the field persisted in CLI output | Medium |
