...
| F3 | **Withdrawn 20260910.** `tests/google_drive/` is gitignored at `.gitignore:83` and tracks no files, so `gdrive_probe.py` exists only in a working tree and is not the repository's text. The findings recorded here against it — a retired version header, a stale process step, a `Usage:` block — were read from an untracked file. | `git ls-files tests/google_drive/` returns nothing | — |
...
| F8 | Both provider modules state `Do not instantiate directly — use get_provider_manager().get_provider('claude')`. It carries no command and no flag, which is why this study read it as compliant — but it is an imperative addressed to a reader, and DR2 and Edit 1 forbid a header that instructs. The verdict is withdrawn; the spec's §4.3 restates both headers descriptively. The lesson is that this study judged by artifact rather than by mood, and the census categories inherited that. | `workmain/ai/providers/claude.py:5`, `gemini.py:5` | — |
