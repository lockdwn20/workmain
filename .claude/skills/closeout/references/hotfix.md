...
| 4 | `git checkout main && git merge --no-ff <branch>`; push `main` — §2.3 | `git merge-base --is-ancestor <branch> main` succeeds and `main` equals `origin/main` |
...
| 6 | `git checkout dev && git merge --no-ff <branch>`; push `dev` — §2.1 | `git merge-base --is-ancestor <branch> dev` succeeds and `dev` equals `origin/dev` |
