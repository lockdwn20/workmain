...
| 5 | `git tag v<version>` on `main`; push the tag; `gh release create v<version> --generate-notes` — `docs/DEVELOPMENT_STANDARDS.md` §2.2 | `git ls-remote --tags origin v<version>` returns the tag and `gh release view v<version>` exits `0` |
| 6 | `git checkout dev && git merge --no-ff <branch>`; push `dev` — `docs/DEVELOPMENT_STANDARDS.md` §2.2, §2.3 | `git merge-base --is-ancestor <branch> dev` succeeds and `dev` equals `origin/dev` |
| 7 | `systemctl --user restart workmain-notify.service` — `docs/DEVELOPMENT_STANDARDS.md` §2.6 | `ActiveEnterTimestamp` postdates the step 6 merge commit |
| 8 | `git branch -d <branch>` — `docs/DEVELOPMENT_STANDARDS.md` §2.3 | `git rev-parse --verify <branch>` fails |
